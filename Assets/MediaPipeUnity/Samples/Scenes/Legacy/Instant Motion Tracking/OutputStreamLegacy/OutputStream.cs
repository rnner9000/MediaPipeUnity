...
namespace LegacyIO
{
  ...
  public class OutputStream<TPacket, TValue> where TPacket : Packet<TValue>, new()
  {
    ...
    protected bool CanCallNext(bool allowBlock)
    {
      if (_poller == null)
      {
        Mediapipe.Logger.LogWarning("OutputStreamPoller is not initialized. Call StartPolling before running the CalculatorGraph");
        return false;
      }
      ...
      {
        Mediapipe.Logger.LogWarning("Cannot avoid thread being blocked when `presenceStreamName` is not set");
        return false;
      }
      return true;
    }
    ...
    protected static bool Next<T>(OutputStreamPoller<T> poller, Packet<T> packet, string stream)
    {
      ...
      {
        Mediapipe.Logger.LogWarning($"Failed to get next value from {stream}. See logs for more details");
        return false;
      }
      return true;
    }
