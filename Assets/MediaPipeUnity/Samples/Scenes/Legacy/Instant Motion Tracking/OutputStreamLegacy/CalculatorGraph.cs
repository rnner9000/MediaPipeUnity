...
namespace LegacyIO
{
  public class CalculatorGraph : MpResourceHandle
  {
    ...
    public CalculatorGraph(Mediapipe.CalculatorGraphConfig config) : this(config.ToByteArray()) { }

    public CalculatorGraph(string textFormatConfig) : this(Mediapipe.CalculatorGraphConfig.Parser.ParseFromTextFormat(textFormatConfig)) { }
    ...
    public Status Initialize(Mediapipe.CalculatorGraphConfig config)
    {
    ...
    public Status Initialize(Mediapipe.CalculatorGraphConfig config, SidePacket sidePacket)
    {
      var bytes = config.ToByteArray();
    ...
    public Mediapipe.CalculatorGraphConfig Config()
    {
      UnsafeNativeMethods.mp_CalculatorGraph__Config(mpPtr, out var serializedProto).Assert();
      GC.KeepAlive(this);

      var config = serializedProto.Deserialize(Mediapipe.CalculatorGraphConfig.Parser);
